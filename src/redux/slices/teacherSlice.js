...
export const extractQuestionsFromPdf = createAsyncThunk(
  ...
  async ({ subjectId, pdfFile }, thunkAPI) => {
    try {
      ...
      console.warn('Backend extract-pdf endpoint failed, using client-side fallback parsing:', error);
      
      // Fallback: Read PDF client-side so user can review and save questions without blocking
      try {
        const clientParsedQuestions = await new Promise((resolve) => {
          ...
          reader.onload = (e) => {
            try {
              ...
              const matches = rawText.match(/[\x20-\x7E\u0600-\u06FF]{4,}/g) || [];
              const cleanText = matches.join(' ');
              const sentences = cleanText.split(/(?=[0-9]+\.|\?)/).filter(l => l.trim().length > 8);

              const items = [];
              for (let i = 0; i < Math.min(sentences.length, 10); i++) {
                const qText = sentences[i].trim().replace(/^[\d+.\s]+/, '');
                if (qText.length > 5) {
                  items.push({
                    text: qText,
              ...
              if (items.length === 0) {
                const name = pdfFile?.name ? pdfFile.name.replace(/\.[^/.]+$/, '') : 'PDF Question';
                items.push({
                  text: `${name} - Question 1`,
                  options: ['Option A', 'Option B', 'Option C', 'Option D'],
                  correctOption: 0,
                  correctAnswer: 0,
                  difficulty: 'medium',
        ...
        if (clientParsedQuestions && clientParsedQuestions.length > 0) {
          return { questions: clientParsedQuestions };
        }
      } catch (fallbackErr) {
        console.error('Client fallback parsing error:', fallbackErr);
      }
