...
const CourseLessons = () => {
      ...
      >
        <div className="flex items-start gap-4">
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-blue-400 shrink-0 shadow-inner ${
            ...
            {isDone ? <FiCheckCircle size={22} /> : <FiPlay size={20} className="fill-blue-400/20 ml-0.5" />}
          </div>
  ...
  return (
    <DashboardLayout
      role="student"
      activeTab="courses"
      title={subject?.name || "Course Lessons"}
      ...
      <div className="flex flex-col gap-6 text-left p-6 md:p-8 pb-32 lg:pb-12 w-full max-w-4xl mx-auto">
        ...
        <div className="p-5 rounded-2xl bg-gray-900/40 border border-gray-800/80 flex flex-col gap-3 shadow-lg">
          <div className="flex items-center justify-between text-sm font-black">
            <span className="text-gray-400 uppercase tracking-widest font-bold">Overall Course Completion</span>
        ...
        <div className="flex flex-col gap-6 text-left">
          <div className="flex items-center justify-between border-b border-gray-800/60 pb-3">
            <h4 className="text-sm font-black text-gray-300 uppercase tracking-widest flex items-center gap-2">
              <FiBookOpen className="text-blue-400" /> Units & Curriculum
            </h4>
            <span className="text-xs font-bold text-gray-400">
              {rawUnits.length} Unit(s) • {totalLessons} Lesson(s)
            </span>
          </div>
          ...
          {isLoading ? (
                ...
                return (
                  <div key={unitId} className="rounded-3xl border border-gray-800/80 bg-[#0c0d19]/40 overflow-hidden transition-all shadow-md">
                    ...
                    <button
                      ...
                      className="w-full p-5 bg-gradient-to-r from-[#121426] to-[#0c0d19] hover:from-[#181a33] flex items-center justify-between gap-4 transition-all text-left cursor-pointer border-b border-gray-800/40"
                    >
                      ...
                      <div className="flex items-center gap-4">
                        <span className="px-3 py-1 rounded-full bg-gray-800/80 border border-gray-700 text-xs font-extrabold text-gray-300">
                          {unitCompleted} / {unitLessons.length} Done
                        </span>
              ...
              {unassignedLessons.length > 0 && (
                <div className="rounded-3xl border border-gray-800/80 bg-[#0c0d19]/40 overflow-hidden transition-all shadow-md">
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      <span className="px-3 py-1 rounded-full bg-gray-800/80 border border-gray-700 text-xs font-extrabold text-gray-300">
                        {unassignedLessons.filter(l => l.isCompleted).length} / {unassignedLessons.length} Done
                      </span>
                      <div className="text-gray-400">
                        {expandedUnits['unassigned'] !== false ? <FiChevronDown size={22} /> : <FiChevronRight size={22} />}
                      </div>
                    </div>
                  </button>
            ...
            <div className="p-8 text-center bg-[#0c0d19]/40 border border-gray-800/80 rounded-3xl flex flex-col items-center justify-center">
              <span className="text-sm font-bold text-gray-400">No lessons uploaded for this course yet.</span>
            </div>
          )}
        </div>
      </div>
