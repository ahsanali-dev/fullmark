...
const TeacherPdfUpload = () => {
  ...
  const [processingStatus, setProcessingStatus] = useState('Uploading PDF...');
  ...
  const handleStartExtraction = async () => {
    ...
    const timer2 = setTimeout(() => {
      ...
      setProcessingStatus('Identifying questions & options with AI...');
    }, 2200);
    ...
    try {
      ...
      setProcessingStatus('AI review complete ✓');
      ...
      const formatted = rawList.map((q, idx) => ({
        ...
        text: q.text || q.question || `Extracted Question ${idx + 1}`,
      ...
      clearTimeout(timer1);
      clearTimeout(timer2);
      toast.error(err || 'Failed to extract questions from PDF');
      setCurrentStep(1);
    }
  };
  ...
  const handleSaveAllQuestions = async () => {
    ...
    setIsSaving(true);
    const toastId = toast.loading(`Saving ${extractedQuestions.length} questions...`);
    ...
    try {
      ...
      toast.success(`Successfully saved ${extractedQuestions.length} questions!`);
      dispatch(fetchQuestions());
      setCurrentStep(4);
    } catch (err) {
      toast.dismiss(toastId);
      toast.error(err || 'Failed to save questions');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <DashboardLayout
      role="teacher"
      activeTab="questions"
      title="PDF Question Upload"
      subtitle="AI-powered question extraction"
    >
      <div className="w-full max-w-4xl mx-auto p-4 md:p-8 pb-32 flex flex-col gap-6 text-left animate-fade-in">
        ...
        {currentStep === 1 && (
          ...
          >
            ...
            <div className="flex flex-col gap-2">
              <label className="text-base font-extrabold text-white">Select Subject</label>
              <div className="relative">
                <FiBookOpen className="absolute left-4 top-1/2 -translate-y-1/2 text-blue-400" size={18} />
                <select
                  value={selectedSubjectId}
                  onChange={(e) => setSelectedSubjectId(e.target.value)}
                  className="w-full pl-12 pr-10 py-4 bg-[#0e101a] border border-gray-800 hover:border-gray-700 rounded-2xl text-white text-base font-bold outline-none focus:border-blue-500 appearance-none cursor-pointer shadow-lg transition-all"
                >
                  {isSubjectsLoading ? (
                ...
                <FiChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" size={20} />
              </div>
            </div>
            ...
            {pdfFile ? (
              <div className="p-6 md:p-8 bg-[#0c0e1a] border-2 border-blue-500/50 rounded-[2.5rem] shadow-[0_0_30px_rgba(37,99,235,0.15)] flex flex-col items-center justify-center text-center relative overflow-hidden group">
                ...
                <button
                  ...
                  className="absolute top-4 right-4 w-9 h-9 rounded-xl bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 flex items-center justify-center transition-all cursor-pointer shadow-sm"
                ...
                <div className="relative w-16 h-16 md:w-20 md:h-20 rounded-3xl bg-blue-600/20 border border-blue-500/40 flex items-center justify-center text-blue-400 mb-3 shadow-[0_0_25px_rgba(59,130,246,0.3)]">
                  <FiFileText size={38} />
                  <span className="absolute -bottom-1 -right-1 px-2 py-0.5 rounded-md bg-blue-600 text-white text-[10px] font-black uppercase tracking-wider shadow-md">
                    PDF
                  </span>
                </div>
              ...
              >
                ...
                <h3 className="text-xl md:text-2xl font-black text-white">Upload PDF File</h3>
                <p className="text-sm font-semibold text-gray-400 mt-1">Tap to browse your PDF</p>
                <button
                  type="button"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-5 bg-[#0e101a] border border-gray-800/80 rounded-2xl flex items-start gap-4 shadow-lg">
                ...
                <div>
                  <h4 className="text-base font-bold text-white leading-tight">AI Extraction</h4>
                  <p className="text-xs font-semibold text-gray-400 mt-1 leading-snug">
                    Automatically detects questions, options and answers
                  </p>
                </div>
              </div>
              ...
              <div className="p-5 bg-[#0e101a] border border-gray-800/80 rounded-2xl flex items-start gap-4 shadow-lg">
                ...
                <div>
                  <h4 className="text-base font-bold text-white leading-tight">Image Support</h4>
                  <p className="text-xs font-semibold text-gray-400 mt-1 leading-snug">
                    Extracts embedded images and diagrams from PDF
                  </p>
                </div>
              </div>
              ...
              <div className="p-5 bg-[#0e101a] border border-gray-800/80 rounded-2xl flex items-start gap-4 shadow-lg">
                ...
                <div>
                  <h4 className="text-base font-bold text-white leading-tight">Review First</h4>
                  <p className="text-xs font-semibold text-gray-400 mt-1 leading-snug">
          ...
          >
            ...
            <div>
              ...
              <div className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-blue-600/15 border border-blue-500/30 text-blue-400 text-xs font-black mt-3">
                <span>Subject: {selectedSubject?.title || selectedSubject?.name || 'Selected Subject'}</span>
              </div>
            ...
            {/* Checklist */}
            <div className="w-full max-w-md flex flex-col gap-3 mt-4 text-left">
              <div className={`p-4 rounded-2xl border flex items-center gap-3 transition-colors ${
                ...
                <span className="text-sm font-bold">Uploading PDF...</span>
              </div>
              ...
              <div className={`p-4 rounded-2xl border flex items-center gap-3 transition-colors ${
                ...
                <span className="text-sm font-bold">Extracting text content...</span>
              </div>
              ...
              <div className={`p-4 rounded-2xl border flex items-center gap-3 transition-colors ${
                ...
                <span className="text-sm font-bold">Identifying questions...</span>
              </div>
            </div>
          </motion.div>
        )}
          ...
          >
            ...
            {extractedQuestions.length === 0 ? (
              <div className="p-12 text-center bg-[#0e101a] border border-gray-800 rounded-3xl">
                <p className="text-gray-400 font-bold">No questions found in this PDF.</p>
                <button
              ...
              <div className="flex flex-col gap-6">
                {extractedQuestions.map((q, qIdx) => (
                  <div
                    key={q._tempId || qIdx}
                    className="p-6 bg-[#0e101a] border border-gray-800/90 rounded-[2rem] shadow-xl flex flex-col gap-5 relative text-left"
                  >
                    {/* Header */}
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <textarea
                        ...
                        className="w-full p-4 bg-[#121424] border border-gray-800 rounded-2xl text-white text-base font-bold outline-none focus:border-blue-500 resize-none"
                      />
                    </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <input
                              ...
                              className="w-full bg-transparent border-none text-white text-sm font-semibold outline-none focus:ring-0"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-2 border-t border-gray-800/40">
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-wider">Difficulty</label>
                        <select
                          value={q.difficulty}
                          onChange={(e) => handleUpdateQuestion(qIdx, 'difficulty', e.target.value)}
                          className="px-3 py-2 bg-[#121424] border border-gray-800 rounded-xl text-white text-xs font-bold outline-none cursor-pointer"
                        >
                      ...
                      <div className="md:col-span-2 flex flex-col gap-1">
                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-wider">Explanation (Optional)</label>
                        <input
                          type="text"
                          value={q.explanation}
                          onChange={(e) => handleUpdateQuestion(qIdx, 'explanation', e.target.value)}
                          placeholder="Brief explanation for correct option"
                          className="px-3 py-2 bg-[#121424] border border-gray-800 rounded-xl text-white text-xs font-semibold outline-none focus:border-blue-500"
                        />
                      </div>
                    </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4 border-t border-gray-800">
              ...
              >
                <FiCheck size={18} />
                <span>Approve & Save {extractedQuestions.length} Questions</span>
              </button>
            </div>
          </motion.div>
        )}
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-gray-400 mt-2 max-w-md">
                Questions have been successfully extracted and added to your Question Bank for{' '}
                <span className="text-blue-400 font-bold">{selectedSubject?.title || selectedSubject?.name || 'Subject'}</span>.
              </p>
            </div>
