        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <p className={`text-sm font-extrabold leading-tight ${isLight ? 'text-[#0f172a]' : 'text-white'}`}>
              {exam.exam?.title || 'Exam Attempt'}
            </p>
            <p className="text-[11px] text-gray-500 font-semibold mt-0.5">
              {exam.subject?.name || 'Subject'} · {formattedDate}
            </p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-4 px-4 py-3">
        ...
        <div className="flex-1 flex flex-col gap-1.5">
          ...
          <p className="text-[11px] text-gray-500 font-semibold text-left">
            {exam.score || 0}% score · Duration: {exam.durationMinutes || 0} mins
          </p>
        </div>
...
const ParentAnalysis = () => {
  ...
  return (
    <DashboardLayout
      ...
      title="Analysis"
      subtitle={`${selectedChild?.name || ''}'s performance`}
      showBackButton
            ...
            {activeTab === 'Overview' && (
              <div className="flex flex-col gap-5 animate-fade-in">
                ...
                <div className="rounded-2xl bg-gradient-to-br from-emerald-600 to-teal-600 p-5 relative overflow-hidden shadow-[0_0_20px_rgba(16,185,129,0.2)]">
                  ...
                  <p className="text-white/70 text-xs font-semibold text-left">Overall Performance</p>
                  <p className="text-4xl font-black text-white mt-1 text-left">{avgScore}%</p>
                  <p className="text-white/80 text-sm font-semibold mt-2 text-left">{performanceLabel}</p>
                  <div className="flex items-center gap-6 mt-3">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-white/60 text-xs font-semibold block ml-0.5 text-left">Streak</span>
                    </div>
                  </div>
                </div>
                ...
                {childSubjects.length > 0 && (
                  <div className="grid grid-cols-2 gap-4">
                    {strongestSubject && (
                      <div className={`rounded-2xl border p-4 flex flex-col gap-3 text-left ${isLight ? 'bg-emerald-50 border-emerald-200' : 'bg-[#111520] border-emerald-500/20'}`}>
                        ...
                        <p className="text-sm font-black text-emerald-400">{Math.round(strongestSubject.averageScore || 0)}% avg</p>
                      </div>
                    )}

                    {weakestSubject && (
                      <div className={`rounded-2xl border p-4 flex flex-col gap-3 text-left ${isLight ? 'bg-red-50 border-red-200' : 'bg-[#111520] border-red-500/20'}`}>
                        ...
                        <p className="text-sm font-black text-red-400">{Math.round(weakestSubject.averageScore || 0)}% avg</p>
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="flex flex-col gap-3">
                  <h3 className={`text-base font-black ${textPrimary} text-left`}>Recent Exams</h3>
                  {isLoading && childResultsData?.attempts?.length === 0 ? (
                    ...
                    childResultsData?.attempts?.slice(0, 5).map((exam) => (
                      <ExamCard key={exam._id} exam={exam} isLight={isLight} />
                    ))
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4 animate-fade-in">
                {isLoading && childSubjects.length === 0 ? (
                  ...
                  childSubjects.map((subj) => {
                    ...
                    // Find attempts for this subject to build individual exam scores
                      ...
                      <div key={subj._id} className={`rounded-2xl border overflow-hidden ${card}`}>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                              ...
                              <p className="text-[11px] text-gray-500 font-semibold">
                                {subj.totalExamsTaken || 0} exams taken
                              </p>
                            </div>
                          </div>
                          <div className="flex flex-col items-end gap-1">
                            <span className="text-base font-black text-yellow-400">{subAvg}% avg</span>
                          </div>
                        </div>
                        ...
                        <div className="px-4 py-3 text-left">
                          ...
                          {subjectAttempts.length === 0 ? (
                            <p className="text-xs text-gray-500 font-semibold">No exam scores available for this subject.</p>
                          ) : (
                            <div className="flex gap-2 flex-wrap">
                              {subjectAttempts.map((attempt, idx) => {
                                ...
                                return (
                                  ...
                                  >
                                    <span className="text-[10px] font-bold text-gray-450 uppercase">Exam {idx + 1}</span>
                          ...
                          <div className="mt-4">
                            <div className="flex items-center justify-between mb-1.5">
                              <span className="text-[11px] font-bold text-gray-550">Average:</span>
